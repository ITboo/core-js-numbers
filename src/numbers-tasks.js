...
function toNumber(value, def) {
  return Number.isFinite(Number(value)) ? Number(value) : def;
}
...
function getFibonacciNumber(index) {
  const first = (1 + 5 ** 0.5) / 2;
  const second = (1 - 5 ** 0.5) / 2;
  const result = Math.trunc((first ** index - second ** index) / 5 ** 0.5);
  return result;
}
...
function isPowerOfTwo(num) {
  return Math.log2(num) === Math.trunc(Math.log2(num));
}
